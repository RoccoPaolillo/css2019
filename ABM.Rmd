...
We built an agent-based model of residential relocation within a town where we specify agents and space based on its geography and demographic data. 
In the model, each *agent* is demographically specified by *ethnicity* ("WHITEB", "ASIAN", "BLACK", or "OTHER"), *socio-economic status* ("LOW", "MID", or "HIGH"), and an individual *threshold* value which quantifies the minimal fractions of similar neighbors the agents needs to perceive positive utility from their residence.
...
where $P_i^X$ is the *fraction of the population with the same ethnicity*, $P_i^Y$ the *fraction of the population with the same socio-economic status*, and $\varepsilon$ is *unobservable utility* of the agent. 
...
The parameters $\beta_\text{Eth}$ and $\beta_\text{SES}$ specifiy the importance of ethnic and respectively socio-economic similarity. 
The higher the threshold $\theta$ the larger is the tendency that utility becomes negative. 
Agents with a high threshold thus have a higher tendency to search an alternative residence in step one of the decision to relocate. In step two of the relocation decision, agents prefer alternatives with either more ethnic similarity or more socio-economic similarity or both. 
When, e.g., $\beta_\text{SES} > \beta_\text{Eth}$ a small increase in socio-economic similarity can outweigh even a slightly larger decrease in ethnic similarity. 

In our particular case, it is reasonable to treat the ethnic group OTHER differently because they represent much more diverse groups of the population than WHITEB, ASIAN, or BLACK. 
...
*Unobservable utility* $\varepsilon$ is a random variable, analogous to an error term in regression models. 
...
The lower $\beta_\text{Eth}$ and $\beta_\text{SES}$ are, the stronger is the impact of unobservable utility. 
...
We performed a parameter exploration with the goal to find parameters which make the segregation patterns of Bradford emerge from a counter-factual maximally non-segregated situation. 
That means, for every simulation we loaded the population of Bradford^[We scaled down the population of Bradford by a factor of ten. So, every simulation ran with about 32,180 agents, i.e. on average 105 per LSOA. We tested extensively that simulation runs are similar to simulations with the full population.] and shuffled agents such that their ethnicity and socio-economic status in every district had a composition as the whole town (see the model description for details). 
...
The standard deviation of $\mu_\theta=0.1$ implied that less than 2% if all agents are already satisfied with less than 10% similars, while also less than 2% would demand more than 50% to stop searching for other options. 
...
Also as in reality, the other ethnicities have the lowest dissimilarity.
Interestingly, also their dissimilarity increases over time in the simulation also all these agents do not take ethnic similaritiy into account in their decision of relocation (which is different from the other three groups). 
...
There are four basic mechanism which make it possible that the fairly high segregation between Asians and white British can emerge from a spatially equalized situation. 
  ...
  (2) Once an agent decided to search for a new location it is more likely to move to a district with more of its kind (in ethnicity or socio-economic status). Such a move would happen even if the agent would remain unsatisfied aftwards because of still low observed utility. 
  ...
  (3) Once the fraction of Asians in a district exceeds the mean threshold the Asians there are more likely to not search anymore for other alternatives because the ethnicity fraction of their neighborhood contributed positively to their utility. 
...
In stochastic models, equilibria are usually stochastic which means that random fluctuation persist in equilibrium while only distributions and their aggregate characteristics stabilize.^[Even in deterministic systems, an equilibrium state need not be static but can by a regular oscillation or even deterministic chaos. Both play no role in our model. Oscillations haven't been observed in the exploration and chaos would be blurred by stochastic fluctuations anyway.] 
...
The main focus of Figure ABM2 is how the simulations evolves with different weights for similarity of ethnicity in the neighborhood ($\beta_{Eth}=4,8,12$) when preferences for similar socio-economic status are ignored ($\beta_\text{SES} = 0$). 
Further on, simulations ran with heterogeneous ($\sigma_\theta=0.1$) thresholds as in the baseline constellation or a homogeneous threshold ($\sigma_\theta=0$). 
...
The opposite is the case, because the random fluctuations help that mechanism (4) kicks in and some districts reach the threshold that white British massively move out. 
This creates a seed of all-Asian districts which makes neighboring districts attractive for Asians. 
With higher $\beta_\text{Eth}$ the simulation does not produce conditions that mechanism (4) can kick in. 
...
Only after at about 800 time steps the dissimilarity of Asians starts to quickly increase and the whole town shifts to the fully segregated situation. 
...
The basic conclusions of this agent-based model is that mild preferences for ethnic similarity can trigger strong ethnic segregation in Bradfords population through the mechanisms (1)--(4). 
